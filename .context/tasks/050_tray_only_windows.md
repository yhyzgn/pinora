...
- 已将托盘失败转换为 `CapabilityUnavailable`，不再启动无窗口、无托盘的后台进程；新增该分支的定向测试。本地通过 app 122 个、core 55 个单元测试（2 个真实桌面测试忽略）、fmt、workspace check、严格 Clippy、diff 与 ctx validate；GitHub CI `30732906042` 已在 Linux、macOS、Windows 原生 runner 通过格式、workspace 编译、严格 Clippy 和单元测试。上述 CI 不含 GUI 会话，真实任务栏、Dock、KWin 与其他 Wayland 合成器探针仍未运行。

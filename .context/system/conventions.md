...
- `pinora-export` 现唯一拥有 `capture_export`、`export_contract`、`image_sink` 与 `export_job` 的导出来源、
  Overlay 完成意图、动作分类、冻结输出目标、标注合成、图像编码、原子保存、系统剪贴板和受监督导出 worker；
  `pinora-app` 仅消费契约和服务接口，且只在 `export_coordination` 持有历史、任务状态和 tray 协调所需状态。
...
126 历史保留期时间策略已完成：`pinora-history::retention` 现唯一拥有当前 Unix 毫秒读取、
零天不读取时钟的保守语义、天数到毫秒的有界转换和饱和截止时间回推；app 仍独占
`reconcile_history_policy` 的启动、设置保存和新增受管历史后的调用时机，以及窗口刷新、
错误反馈、Window/Surface 和唯一 EventLoop。`cargo test -p pinora-history -- --nocapture`（32
通过）、app 回归（25 通过）、完整 workspace、Clippy、Windows target、`--version`、fmt、diff
和上下文校验均通过。真实时钟跳变、文件系统、GUI、tray-only 和性能仍未验证。
...
138 导出请求契约 crate 已完成：`pinora-export::export_contract` 唯一拥有 `OverlayExportAction`、贴图强制
标注图来源选择、`ExportAction`、`ExportOperation` 和 `FrozenExportTarget`；其生产依赖只含既有
`pinora-core`、`pinora-jobs`、`image` 与 `png`，不依赖 app、history、desktop 或 winit。
`pinora-app::export_coordination` 仅保留 `PendingExport`、历史候选、运行中文件保存筛选、结果资产门禁和
tray 映射，`desktop_shell` 继续独占运行时、文件名、任务、结果、Window/Surface、EventLoop 和全部导出副作用。
`cargo test -p pinora-export -- --nocapture`（33 通过，1 项真实剪贴板会话忽略）、
...
`pinora-core`，`pinora-jobs` 仅作测试依赖，不依赖 winit。`desktop_shell` 继续独占 Overlay Window/Surface、
绘制、输入、标注文档写入、OCR/导出任务、tray 和 EventLoop。`cargo test -p pinora-overlay -- --nocapture`
（3 项）、`cargo test -p pinora-app --lib -- --nocapture`（24 项）、
`PINORA_NO_SYSTEM_CLIPBOARD=1 cargo test --workspace`、`cargo check --workspace`、严格 Clippy、Windows target、

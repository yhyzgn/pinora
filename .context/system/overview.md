...
- 2026-08-02 的 057 本地实现为 `CaptureProvider` 增加了经快照验证的窗口枚举/捕获契约：xcap 在点击后按内部窗口 ID、几何、显示器和缩放重新枚举验证，最小化、消失或拓扑变化不会回退为显示器截图；tray 仅在启动时保存最多 20 个经过清洗的候选。窗口捕获和 Overlay 创建失败会回到 tray 空闲态，后台线程只回传稳定错误码，避免后端错误文本写入日志。xcap/fake/托盘/桌面壳离线测试及本地静态门禁已通过；真实窗口像素、权限、任务栏/Dock 与合成器行为尚未验证。

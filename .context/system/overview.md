...
- 2026-08-02 的 058 本地实现取消了 `LoadingState` 向 `about_to_wait` 返回可恢复错误的路径：区域、全屏、指定显示器、窗口和延时截图的 worker 错误、断开、预览不一致和 Overlay 建立失败均释放当前会话并回到 tray；延时会话保留贴图恢复优先级，日志只输出稳定错误码。全量离线门禁已通过；真实桌面窗口销毁、tray 连续驻留和任务栏/Dock 行为仍未验证。

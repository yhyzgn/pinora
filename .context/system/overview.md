...
- 2026-08-03 的 133 将 `OverlayPhase`、`OverlayAssetIdentity`、选区标注 revision 到 `AssetRef`
  的映射和派生 `CaptureImage` 身份盖章迁入 `pinora-app::overlay_session`。模块只依赖
  `pinora-core`，不依赖 winit；`desktop_shell` 继续独占 Overlay Window/Surface、绘制、输入、
  标注文档、OCR/导出任务、tray 与 EventLoop。状态模块 3 项、app 27 项、workspace 回归、严格 Clippy、
  Windows target、版本探针、格式、差异和上下文校验通过；离线证据不证明真实 GUI、窗口管理器、
  焦点、任务栏/Dock、HiDPI、tray-only 或性能。

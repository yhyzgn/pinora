...
- 2026-08-03 的 098 发布链路预验证已通过：CI run `30782856266` 在 Linux/macOS/Windows 原生 runner 完成质量门禁；手动 package run `30783019714` 生成三平台 raw/压缩包/可用安装包和逐平台 `SHA256SUMS.txt`，本地下载后逐项校验通过；runtime-verify run `30783253220` 对同一 package run 完成 Linux tar/deb、macOS app、Windows zip/NSIS（存在时）的 `--version` smoke，并生成 runtime 报告。该证据不涵盖真实 GUI、tray、热键、权限、任务栏/Dock/分页器、签名/公证或性能；tag pre-release 尚待本任务完成。

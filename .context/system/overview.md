...
- `cargo fmt --check`、`cargo check --workspace` 和 `cargo clippy --workspace --all-targets -- -D warnings` 已于 2026-08-02 通过；`cargo test --workspace` 通过 136 个可执行单元测试（86 app、50 core），另有 2 个真实桌面测试被忽略；仍没有 GUI 端到端测试。
...
- `pinora-core::settings` 与 `pinora-app::SettingsStore` 已于 2026-08-02 建立版本化设置与原子文件基础：格式有 magic、schema 与长度校验，非法数值逐字段修复，损坏/未知版本保留源文件并回退内存默认。启动只读 Linux XDG/HOME 默认路径，不自动写入；设置 UI、实际运行时应用和跨平台目录策略仍未交付。
...
- 选定并验证的 UI Adapter、完整工具条、OCR 拖选编辑器、跨屏联合 Overlay、设置 UI/运行时应用、历史 UI/文件删除事务、真实透明/点击穿透，以及 Windows/macOS/X11/通用 Wayland 的正式适配与桌面验证。

...
- `cargo fmt --check`、`cargo check --workspace` 和 `cargo clippy --workspace --all-targets -- -D warnings` 已于 2026-08-02 通过；`cargo test --workspace` 通过 139 个可执行单元测试（89 app、50 core），另有 2 个真实桌面测试被忽略；仍没有 GUI 端到端测试。

...
- 任务 105 已将启动项、单实例/IPC、全局热键和 Linux Wayland Portal 迁入 `pinora-platform`；任务 106 又将 KDE/Spectacle、xcap、显式 fake、后端选择和 `FrameCache` 迁入 `pinora-capture`。`pinora-app` 不再直接声明 `fs2`、`global-hotkey`、`xcap`、`zbus`、`async-channel` 或 `futures-lite`；任务监督、存储、导出、OCR、Overlay/贴图、托盘和窗口编排仍在 `pinora-app`，后续按功能任务继续拆分。

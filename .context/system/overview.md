...
| 后台驻留与窗口隔离 | 启动后只保留托盘、可用全局热键、IPC 与帧缓存，不自动截图；无法创建托盘时以 `CapabilityUnavailable` 退出；Overlay、贴图、历史、设置及兼容窗口统一请求跳过任务栏/Dock，真实桌面验证仍待完成 |
...
- `cargo fmt --check`、`cargo check --workspace` 和 `cargo clippy --workspace --all-targets -- -D warnings` 已于 2026-08-02 通过；当前 `PINORA_NO_SYSTEM_CLIPBOARD=1 cargo test --workspace` 通过 app 131 个、core 55 个单元测试，另有 2 个真实桌面测试被忽略；仍没有 GUI 端到端测试。
...
- GitHub Actions CI `30732620836`、`30732765136` 与 `30732906042` 已于 2026-08-02 在 Linux、macOS、Windows 原生 runner 通过格式、workspace 编译、严格 Clippy 和单元测试；这些运行未创建 GUI 会话，不能作为任务栏、Dock、窗口交互或 KWin 行为的证据。

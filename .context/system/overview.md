...
- Rust 2024 workspace：`pinora`（`src/main.rs`）+ `pinora-core` + `pinora-platform` + `pinora-app`，并按功能拆分 `pinora-diagnostics`、`pinora-panels` 等 crate。
...
- 2026-08-03 的 113 将设置/历史/诊断面板、Overlay 选区读数和贴图客户区菜单迁入 `pinora-desktop`；`pinora-app` 通过 crate 内兼容 re-export 复用这些纯 UI 模块，`cargo tree -p pinora-desktop --depth 1` 仅显示 `pinora-core` 与 `winit`，`cargo tree -p pinora-app --depth 1` 仍显示对 `pinora-desktop` 的直接依赖；`cargo test -p pinora-desktop -- --nocapture` 77 项全部通过，随后又通过了 `cargo fmt --check`、`cargo check --workspace`、`cargo clippy --workspace --all-targets -- -D warnings`、`git diff --check` 和 `ctx validate`。该证据只覆盖离线布局、命中和绘制，不证明真实 GUI、HiDPI、输入法、焦点、任务栏/Dock 或性能。
- 2026-08-03 的 114 将 `image_sink` 与 `export_job` 迁入 `pinora-export`；`pinora-app` 仍由唯一 EventLoop 编排请求、结果门禁反馈和关闭回收。`cargo test -p pinora-export -- --nocapture` 25 项通过、1 项真实系统剪贴板测试忽略；该证据不证明真实跨平台剪贴板权限、桌面 GUI 或性能。
- 2026-08-03 的 115 将 `history_export` 与 `history_load_job` 迁入 `pinora-history`；`pinora-app` 仅通过 crate re-export 消费历史策略和异步读取服务，历史窗口资源/Panel/选择状态仍留在 app。`cargo test -p pinora-history -- --nocapture` 26 项全部通过；该证据不证明真实文件权限、断电、网络文件系统、GUI 或性能。
- 2026-08-03 的 116 将 `tray.rs` 迁入 `pinora-tray`；Linux GTK 依赖与 `tray-icon` 仅由该 crate 声明，app 仍通过 `TrayAction` 编排截图、贴图、设置、历史和退出。`cargo test -p pinora-tray -- --nocapture` 15 项全部通过；该证据不证明真实跨平台托盘、任务栏/Dock、菜单点击、重连或性能。
- 2026-08-03 的 117 将 `ocr_job.rs` 迁入 `pinora-ocr`；该 crate 现拥有本地 runner、worker 生命周期、进程内 LRU 风格结果缓存及 owner/资产代际/截止时间门禁，app 仅传入当前 UI 资产并消费完成事件。`cargo test -p pinora-ocr -- --nocapture` 26 项与 `cargo test -p pinora-app --lib -- --nocapture` 57 项通过；该证据不证明真实 Tesseract 模型、外部进程压力、GUI 词框交付或性能。
- 2026-08-03 的 118 将 `runtime.rs` 迁入 `pinora-runtime`；该 crate 现拥有 `AppRuntime`、命令分发、单实例 bootstrap/forward/shutdown、能力探测端口和领域事件发布，app 保留真实平台探测实现。`cargo test -p pinora-runtime -- --nocapture` 14 项与 app 43 项回归通过；该证据不证明真实跨平台单实例、权限、桌面窗口隔离或性能。
- 2026-08-03 的 119 将 `desktop_shell` 的纯 XRGB 基础帧缓存、缩放、压暗、脏区恢复和边框/手柄绘制迁入 `pinora-desktop::xrgb`；app 继续持有 Window/Surface 并只在需要时上传帧。`cargo test -p pinora-desktop -- --nocapture` 83 项与 app 41 项回归通过；该证据不证明真实 softbuffer、HiDPI、连续 resize、焦点或性能。
- 2026-08-03 的 120 将 `desktop_shell` 的 Overlay 缓冲/原图/窗口物理像素映射、选区局部标注坐标、选区调整资格和手柄命中迁入 `pinora-desktop::overlay_geometry`；app 继续持有 `OverlayState`、`SelectionSession`、winit 输入、Window/Surface 和 EventLoop。`cargo test -p pinora-desktop -- --nocapture` 87 项与 app 36 项回归通过；该证据不证明真实 winit 缩放、HiDPI、连续输入、焦点、任务栏/Dock 或性能。
- 2026-08-03 的 121 将标注局部框显示投影、脏区裁剪和 XRGB 块拷贝迁入 `pinora-desktop::{overlay_annotation,xrgb}`；app 继续查询标注状态并管理缓存、Window/Surface 和 present。`cargo test -p pinora-desktop -- --nocapture` 91 项与 app 36 项回归通过；该证据不证明真实 softbuffer、HiDPI、连续拖动、焦点、任务栏/Dock 或性能。
- 2026-08-03 的 122 将 `CapturePreview` 迁入 `pinora-capture`：冷捕获与预截帧通过同一值对象移交原始图像、XRGB 基础帧和暗化帧，完整性检查拒绝短缓冲；app 仅处理接收结果、错误、Overlay 目标和 Window/Surface。`cargo test -p pinora-capture -- --nocapture` 27 项通过、1 项真实显示会话测试忽略，app 35 项回归通过；该证据不证明真实捕获权限、窗口、softbuffer、HiDPI、任务栏/Dock 或性能。
- 2026-08-03 的 123 将 `CaptureExportSource` 与 `compose_capture_export_image` 迁入 `pinora-export::capture_export`：原图来源不读取标注，标注来源优先烧录已提交文档，没有已提交文档时才使用草稿预览，异常预览长度回退裁剪原图；app 保留选区裁剪、资产盖章和导出工作流。`cargo test -p pinora-export -- --nocapture` 30 项通过、1 项真实剪贴板测试忽略，app 33 项回归通过；完整 workspace 测试、Clippy、Windows target、fmt、diff 和上下文校验通过。该证据不证明真实剪贴板/文件权限、窗口、HiDPI、任务栏/Dock 或性能。
- 2026-08-03 的 124 将 `CaptureMode`、`CaptureTarget`、`OverlayInitialSelection`、初始选区应用和显示器目标解析迁入 `pinora-capture::capture_request`：区域截图保持手动选区；全屏、全部显示器和窗口截图保持全图选区；显式目标消失返回 `NotFound`，非显示器目标按 `InvalidState` 拒绝而不回退。app 保留请求发起、倒计时、失败恢复、实际捕获和窗口编排。`cargo test -p pinora-capture -- --nocapture` 33 项通过、1 项真实显示会话测试忽略，app 30 项回归通过；完整 workspace 测试、Clippy、Windows target、`--version` 探针、fmt、diff 和上下文校验通过。该证据不证明真实截图权限、窗口、HiDPI、任务栏/Dock 或性能。
- 2026-08-03 的 125 将 `AnnotationHistoryAction`、`TextEnterAction`、撤销/重做、文本 Enter、微调步长和双击复制判定迁入 `pinora-desktop::overlay_input`：Ctrl+Z、Ctrl+Shift+Z/Ctrl+Y、Shift+Enter、Enter、Shift+方向键以及序号/选择工具的双击豁免保持不变。app 保留 winit 事件分发、Overlay 状态、标注文档写入、任务和窗口编排。`cargo test -p pinora-desktop -- --nocapture` 95 项与 app 26 项回归通过；完整 workspace 测试、Clippy、Windows target、`--version` 探针、fmt、diff 和上下文校验通过。该证据不证明真实输入法、焦点、窗口、HiDPI、任务栏/Dock 或性能。
- 2026-08-03 的 126 将当前 Unix 毫秒读取、零天短路、天数到毫秒转换和饱和截止时间回推迁入 `pinora-history::retention`；app 仍在启动、设置保存成功和受管 PNG 新增后调用历史策略，并保留面板刷新、错误反馈、Window/Surface 和唯一 EventLoop。history 32 项与 app 25 项定向测试、完整 workspace、严格 Clippy、Windows target、`--version`、fmt、diff 和上下文校验通过；真实时钟跳变、断电/权限/网络文件系统、GUI、tray-only、任务栏/Dock 和性能仍待原生探针。
- 2026-08-03 的 127 将脱敏诊断报告模型、固定标签校验、字段顺序和原子文件发布迁入 `pinora-diagnostics`；app 仅从 `DiagnosticsPanel`/runtime 组装已审计输入并保留托盘反馈。定向报告测试 5 项、app 回归 22 项通过；真实只读目录、断电、托盘菜单、窗口隔离和性能仍待原生探针。
- 2026-08-03 的 128 将设置、历史和诊断窗口适配器迁入 `pinora-panels`；该 crate 只绑定既有
  Panel、`pinora-desktop::window_policy`、winit/softbuffer 和设置存储，app 继续拥有唯一
  EventLoop、打开/关闭时机、输入业务、设置保存后的副作用、历史加载/删除和诊断导出。
  定向 panels 源码守卫与 app 回归已通过；真实窗口管理器、焦点、任务栏/Dock、HiDPI 和性能仍待原生探针。
- 2026-08-03 的 129 将 `desktop_shell` 的 `Mode`、`LoadingState`、`DelayedCapture`、失败范围和
  屏幕/虚拟桌面/窗口/历史/贴图编辑 Overlay 目标构造迁入 `pinora-app::capture_session`。该模块不依赖
  winit，延时可见性快照只保留领域 `PinId`；app 将 `PinId` 映射回当前窗口后才设置可见性。真实捕获、
  worker、Window/Surface、EventLoop、tray 反馈和失败恢复仍由 `desktop_shell` 独占。状态模块 6 项、
...
- 2026-08-03 的 137 将 `HistoryLoadIntent`、`HistoryLoadRequest`、`ActiveHistoryLoad`、准备类型映射和
  当前历史选择的结果资产门禁从 app 私有模块迁入 `pinora-history::history_session`。模块不读取文件、不启动或
  轮询 worker、不创建窗口或访问 runtime/tray；`desktop_shell` 继续独占 `HistoryLoadJobService`、历史窗口、
  贴图/编辑器、错误反馈和 EventLoop。history 35 项、app 12 项、workspace 回归、严格 Clippy、Windows target、
  版本探针、格式、差异和上下文校验通过；离线证据不证明真实历史目录权限、worker 时序、窗口管理器、焦点、
  HiDPI、tray-only 或性能。
- 2026-08-03 的 139 将 Overlay 的 Window/Surface、隐藏创建、展示、隐藏、焦点、IME、重绘和固定像素尺寸同步
  迁入 `pinora-panels::OverlayWindow`；`OverlayState` 不再直接持有平台资源，app 继续拥有 Overlay 会话、
  绘制、输入、任务、关闭 owner 和唯一 EventLoop。panels 源码策略守卫、app 资源所有权守卫、workspace 回归、
  严格 Clippy、Windows target、版本探针、格式、差异、依赖图和上下文校验通过。离线证据不证明真实任务栏/Dock/
  分页器、tray-only、首帧、焦点、IME、HiDPI 或连续拖拽性能。
...
- 2026-08-03 的 133/134 将 `OverlayPhase`、`OverlayAssetIdentity`、选区标注 revision 到 `AssetRef`
  ...
  输入、标注文档、OCR/导出任务、tray 与 EventLoop。crate 3 项、app 24 项、workspace 回归、严格 Clippy、
  Windows target、版本探针、格式、差异和上下文校验通过；离线证据不证明真实 GUI、窗口管理器、焦点、

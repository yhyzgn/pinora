...
| 当前代码状态 | Rust 2024 workspace；当前实际 crate 为 `pinora-core`、`pinora-platform`、`pinora-capture`、`pinora-jobs`、`pinora-storage`、`pinora-desktop`、`pinora-overlay`、`pinora-pin`、`pinora-ocr`、`pinora-export`、`pinora-history`、`pinora-diagnostics`、`pinora-panels`、`pinora-tray`、`pinora-runtime`、`pinora-app`，桌面能力仍以 Linux/KDE 实验路径为主，尚未达到可发布架构 |
...
### 2.7 `pinora-overlay` Overlay 纯会话边界

`pinora-overlay` 将已经确认选区的纯状态从桌面壳中剥离：它拥有 `OverlayPhase`、本次选区的
`ImageId`、标注 revision 到 `AssetRef` generation 的确定性映射，以及派生图像的身份盖章。
同一选区的标注、撤销和重做只改变 generation；确认重选时由桌面壳创建新的身份，因此迟到的 OCR
或导出结果仍会被现有 owner/asset 门禁拒绝。生产代码只依赖 `pinora-core`，没有窗口、图形表面、
winit、worker、runtime 或 tray 依赖；`pinora-jobs` 仅作为陈旧结果回归的开发依赖。

```mermaid
flowchart LR
    Input["desktop_shell\n确认选区 / 重选 / 标注 revision"] --> Session["pinora-overlay\nOverlayPhase + OverlayAssetIdentity"]
...
- 纯会话模块不决定选区几何，不写入标注文档，也不提交任务；这些操作需要真实输入和窗口时序，仍由
  `desktop_shell` 维持。
- 公开 crate 契约仅包含阶段、资产身份、revision 映射和盖章；它不接受 UI 句柄或平台类型，防止这些类型
  渗入资产版本契约。
...
- 设置项 `start_on_login` 默认关闭，只有用户在设置面板明确保存时才改变平台注册项；注册成功且本地 v9 设置原子保存成功后才提交新 runtime 状态。
- Linux 写入用户级 `$XDG_CONFIG_HOME/autostart/pinora.desktop`（未设置时为 `~/.config/autostart`），Windows 写入当前用户 `HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run`，macOS 写入用户级 `~/Library/LaunchAgents/io.github.yhyzgn.pinora.plist` 兼容路径。
- 所有入口使用 Pinora 所有权标记和绝对可执行路径；启动参数为 `--pinora-autostart`，只启动 tray-only 生命周期，已有实例不触发截图。禁用只移除 Pinora 自己创建的项，冲突项拒绝覆盖/删除。
- 平台写入采用同目录临时文件、同步和原子替换；热键重绑或设置保存失败时尝试恢复旧启动项，失败通过稳定错误码反馈而不报告新设置成功。
...
```mermaid
flowchart LR
    Shell["desktop_shell\n贴图窗口/输入/runtime/OCR/导出/tray/EventLoop"]
    Session["pinora-pin\n鼠标状态/呈现参数/关闭快照/最近使用序号"]
    Core["pinora-core\nCaptureImage、PixelPoint"]
    Window["Window / Surface / winit / worker / tray"]
...
**默认动作**：F1 区域截图、F3 从系统图像剪贴板创建贴图、Shift+F3 显示/隐藏全部贴图；全屏截图仅由托盘和 IPC 提供，不占用默认全局键。

**详细功能**：

- 设置页提供区域截图与剪贴板贴图两项录制控件，显示修饰键、主键、平台显示名和当前占用状态。默认值分别为 F1、F3；Shift+F3 是固定的全部贴图显隐键，不能被覆盖或复用。
- 绑定前做应用内重复检查；注册后接收平台回调并转成稳定 `ActionId`。窗口内键盘路径也按同一份已保存 `HotkeyBinding` 路由，避免设置只对全局后端生效。
- Windows 使用系统热键能力；macOS 使用全局快捷键能力；Linux X11 使用 X11 后端；Wayland 优先使用 XDG GlobalShortcuts Portal。
- Wayland Portal 首次绑定需要展示授权/配置流程和当前后端状态；F1、F3、Shift+F3 三项必须全部被接受才报告可用，不能静默假设部分注册成功。
...
| 键位 | `ActionId` | 详细行为 | 边界 |
| --- | --- | --- | --- |
| F1 | `CaptureRegionAndPin` | 发起现有区域截图会话，等待真实捕获帧后展示 Overlay；用户确认后复用既有贴图、复制、保存和标注流程。 | 平台捕获不可用时只返回受控失败，不创建 fake 图像。 |
| F3 | `PasteClipboard` | 在后台读取系统 `image/png`，完成 MIME、字节、像素与解码校验后，复用 `PinWindow` 创建路径。 | 不在 GUI 线程等待子进程；新 F3 会取消前一未完成读回；不支持、超时、损坏或非图像内容只产生固定失败反馈。 |
| Shift+F3 | `ToggleAllPinsVisibility` | 若任一既有贴图隐藏则显示全部，否则隐藏全部；显示时只走 `window_policy` 的辅助窗口展示入口。 | 不创建贴图、不改变图像和历史；空贴图集合是无副作用操作；不得产生任务栏、Dock 或分页器入口。 |

#### 剪贴板贴图与全部显隐流程图

```mermaid
flowchart TD
    Key["F3 / Shift+F3"] --> Route["统一 HotkeyBinding 路由"]
    Route -->|"F3"| Submit["提交 Clipboard JobSpec\nowner = Clipboard(ImageId)"]
    Submit --> Worker["受监督 worker\nwl-paste image/png -> xclip"]
    Worker --> Validate{"超时、取消、大小与 PNG 校验通过？"}
    Validate -->|"否"| Feedback["Tray 固定失败反馈\n不创建窗口"]
    Validate -->|"是"| Pin["open_pin_from_image"]
    Pin --> Policy["window_policy 展示既有 PinWindow"]
    Route -->|"Shift+F3"| Existing["读取既有 PinWindow 集合"]
    Existing --> Visible{"是否存在隐藏贴图？"}
    Visible -->|"是"| Show["显示全部\nwindow_policy"]
    Visible -->|"否"| Hide["隐藏全部\nset_visible(false)"]
```
...
**托盘菜单**：区域截图、剪贴板贴图、全屏截图、贴图列表、显示/隐藏全部、关闭全部、设置、诊断、退出。
...
- 剪贴板贴图只读取 `image/png`：Linux worker 优先 `wl-paste --type image/png`，再尝试 `xclip`；读取受 3 秒截止时间、64 MiB 原始字节、5,000 万像素和 200 MiB 解码缓冲限制。任一限制、取消或解码失败只在 tray 显示固定错误码，不写回系统剪贴板。
...
下表是**目标验证矩阵**，不是当前支持声明。当前仍以 Linux/KDE 实验路径为主；Windows target 编译已通过，但 macOS/X11/通用 Wayland 没有完整核心流程和真实 GUI 证据。
...
| 开机自启 | 当前用户 Run | 用户级 LaunchAgent 兼容路径 | XDG Autostart | XDG Autostart | 已完成离线契约；真实登录未验证 | 平台失败保持旧值，保留 tray/IPC 手动入口 |

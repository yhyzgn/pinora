...
use pinora_core::{ErrorCode, ExportImageFormat, PinoraError};
...
impl ExportNameAllocator {
        ...
        F: FnMut(&Path) -> Result<bool, PinoraError>,
    {
        ...
        for sequence in first_sequence..=MAX_SEQUENCE {
            let path = export_dir.join(file_name(seconds, sequence, format));
            if !occupied(&path)? {
...
fn file_name(seconds: u64, sequence: u32, format: ExportImageFormat) -> String {
    let (year, month, day, hour, minute, second) = utc_parts(seconds);
    let extension = format.file_extension();
    if sequence == 0 {
        format!("Pinora_{year:04}{month:02}{day:02}_{hour:02}{minute:02}{second:02}.{extension}")
    } else {
        format!(
            "Pinora_{year:04}{month:02}{day:02}_{hour:02}{minute:02}{second:02}_{sequence:03}.{extension}"
        )
    }
}
...
mod tests {
    ...
    fn filename_uses_sortable_utc_time_without_internal_identity() {
        assert_eq!(
            file_name(0, 0, ExportImageFormat::Png),
            "Pinora_19700101_000000.png"
        );
        assert_eq!(
            file_name(951_782_400, 0, ExportImageFormat::Png),
            "Pinora_20000229_000000.png"
        );
        assert_eq!(
            file_name(1_785_681_000, 7, ExportImageFormat::Png),
            "Pinora_20260802_143000_007.png"
        );
        assert_eq!(
            file_name(1_785_681_000, 0, ExportImageFormat::Jpeg),
            "Pinora_20260802_143000.jpg"
        );
        assert_eq!(
            file_name(1_785_681_000, 0, ExportImageFormat::WebP),
            "Pinora_20260802_143000.webp"
        );
        assert!(!file_name(1_785_681_000, 0, ExportImageFormat::Png).contains("ImageId"));
    }
    ...
    fn same_second_and_existing_names_increment_without_overwrite() {
        ...
        let first = allocator
            .allocate(
                ...
                |path| Ok(existing.contains(path)),
            )
    ...
    fn occupied_check_failure_is_returned_without_advancing_state() {
        ...
        let mut allocator = ExportNameAllocator::default();
        let error = allocator
            .allocate(directory, at(0), ExportImageFormat::Png, |_| {
                Err(PinoraError::new(
                    ErrorCode::Internal,
        ...
        let path = allocator
            .allocate(directory, at(0), ExportImageFormat::Png, |_| Ok(false))

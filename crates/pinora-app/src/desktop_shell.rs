...
use crate::overlay_session::{OverlayAssetIdentity, OverlayPhase, overlay_asset_for_revision};
...
fn overlay_current_asset(overlay: &OverlayState) -> Option<AssetRef> {
    overlay_asset_for_revision(overlay.annotation_asset, overlay.annotate.doc.revision())
}
...
struct OverlayState {
    ...
    export_source: CaptureExportSource,
    ...
    S: ImageSink,
{
    ...
    fn queue_history_load(&mut self, intent: HistoryLoadIntent) {
        ...
        self.queued_history_load = Some(HistoryLoadRequest::new(entry, intent));
        if let Some(history) = self.history.as_mut() {
    ...
    fn start_queued_history_load(&mut self) {
        ...
        let still_selected = self
            ...
            .as_ref()
            .and_then(|history| history.panel().selected_entry())
            .is_some_and(|entry| request.matches_entry(entry));
        if !still_selected {
            return;
        }
        ...
        match self.history_load_jobs.start(spec, input) {
            Ok(ticket) => {
                ...
                self.active_history_load = Some(ActiveHistoryLoad::new(ticket.id, request));
            }
            Err(error) => {
    ...
    fn take_active_history_load(&mut self, job_id: JobId) -> Option<HistoryLoadRequest> {
        self.active_history_load
            .as_ref()
            .is_some_and(|active| active.has_job_id(job_id))
            .then(|| {
                self.active_history_load
                    .take()
                    .map(ActiveHistoryLoad::into_request)
            })
            .flatten()
    }
    ...
    fn poll_history_load_jobs(&mut self, event_loop: &ActiveEventLoop) {
        ...
        let completions = self
            ...
            .poll(monotonic_ms(), |job_id, owner| {
                active
                    .as_ref()
                    .and_then(|active| active.accepts_result(selected.as_ref(), job_id, owner))
            });
    ...
    fn open_overlay_with_preview(
        ...
        self.overlay = Some(OverlayState {
            ...
            export_source: CaptureExportSource::default(),
    ...
    fn overlay_ocr(&mut self) {
        ...
        let image = match self.crop_overlay_image(CaptureExportSource::Annotated) {
            Ok(img) => img,
            Err(e) => {
    ...
    fn crop_overlay_image(&self, source: CaptureExportSource) -> Result<CaptureImage, PinoraError> {
        let ov = self
            .overlay
            .as_ref()
        ...
        let mut output = compose_capture_export_image(crop, &ov.annotate, source);
